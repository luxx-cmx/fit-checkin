const K = {
  ...
  SYNC_PENDING: 'syj_sync_pending',
  SYNC_META: 'syj_sync_meta',
  SEARCH_HISTORY: 'syj_search_history',
  COMMON_PORTIONS: 'syj_common_portions',
  CHART_TAGS: 'syj_chart_tags',
  UI_PREFS: 'syj_ui_prefs',
}

const TRASH_KEEP_DAYS = 7
const SYNC_TABLES = ['diet', 'weight', 'health']
const EMPTY_SYNC_PENDING = { diet: [], weight: [], health: [] }
...
function normalizeSyncId(id) {
  return String(id)
}

function normalizeSyncPending(value) {
  return SYNC_TABLES.reduce((acc, table) => {
    const ids = Array.isArray(value?.[table]) ? value[table].map(normalizeSyncId) : []
    acc[table] = Array.from(new Set(ids))
    return acc
  }, { ...EMPTY_SYNC_PENDING })
}

function getSyncPendingState() {
  return normalizeSyncPending(get(K.SYNC_PENDING, EMPTY_SYNC_PENDING))
}

function setSyncPendingState(value) {
  set(K.SYNC_PENDING, normalizeSyncPending(value))
}

function getSyncPendingCount(value = getSyncPendingState()) {
  return SYNC_TABLES.reduce((sum, table) => sum + value[table].length, 0)
}

function getSyncMetaState() {
  return { ...DEFAULT_SYNC_META, ...get(K.SYNC_META, DEFAULT_SYNC_META) }
}

function updateSyncMeta(patch) {
  const next = { ...getSyncMetaState(), ...patch }
  set(K.SYNC_META, next)
  return next
}

function queuePendingSync(table, ids) {
  if (!SYNC_TABLES.includes(table)) return getSyncPendingState()
  const list = Array.isArray(ids) ? ids : [ids]
  const current = getSyncPendingState()
  const next = {
    ...current,
    [table]: Array.from(new Set([...current[table], ...list.filter((id) => id !== undefined && id !== null).map(normalizeSyncId)])),
  }
  setSyncPendingState(next)
  notifySyncState({ table, operation: 'queue', status: 'pending', pendingCount: getSyncPendingCount(next) })
  return next
}

function resolvePendingSync(table, ids) {
  if (!SYNC_TABLES.includes(table)) return getSyncPendingState()
  const list = new Set((Array.isArray(ids) ? ids : [ids]).map(normalizeSyncId))
  const current = getSyncPendingState()
  const next = {
    ...current,
    [table]: current[table].filter((id) => !list.has(id)),
  }
  setSyncPendingState(next)
  return next
}

function failSync(detail) {
  const message = detail?.message || '同步失败'
  updateSyncMeta({
    syncing: false,
    lastOperation: detail?.operation || '',
    lastFailureAt: new Date().toISOString(),
    lastFailureMessage: message,
  })
  notifySyncState({ ...detail, status: 'failed', message, pendingCount: getSyncPendingCount() })
}
...
async function syncFetchWithRetry(table, operation, url, options, retries = 3, hooks = {}) {
  let lastError = null
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      notifySyncState({ table, operation, status: 'success', attempt, pendingCount: getSyncPendingCount() })
  ...
  const message = lastError?.message || '同步失败'
  hooks.onFailed?.(message)
  failSync({ table, operation, retries, message })
  trackEvent('sync_failed', { table, operation, retries, message })
  return false
}

function pushRemote(table, row) {
  const pending = queuePendingSync(table, row?.id)
  const t = token()
  if (!t) {
    failSync({ table, operation: 'push', message: '当前未登录，新增记录仅保存在本地' })
    return pending
  }
  updateSyncMeta({ syncing: true, lastOperation: 'push' })
  notifySyncState({ table, operation: 'push', status: 'syncing', pendingCount: getSyncPendingCount(pending) })
  syncFetchWithRetry(table, 'push', `/api/sync/${table}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${t}` },
    body: JSON.stringify({ rows: [row] }),
  }, 3, {
    onSuccess: () => resolvePendingSync(table, row?.id),
  })
}
...
export const getUiPrefs = () => get(K.UI_PREFS, {})
export const setUiPrefs = (patch = {}) => {
  const next = { ...getUiPrefs(), ...patch, updatedAt: new Date().toISOString() }
  set(K.UI_PREFS, next)
  trackEvent('ui_prefs_saved', { keys: Object.keys(patch) })
  return next
}

export const getSearchHistory = (limit = 10) => get(K.SEARCH_HISTORY, []).slice(0, limit)
export const recordSearchHistory = (keyword) => {
  const text = String(keyword || '').trim()
  if (!text) return getSearchHistory()
  const next = [text, ...get(K.SEARCH_HISTORY, []).filter((item) => item !== text)].slice(0, 10)
  set(K.SEARCH_HISTORY, next)
  trackEvent('search_history_recorded', { keyword: text })
  return next
}
export const clearSearchHistory = () => set(K.SEARCH_HISTORY, [])

export const getCommonPortions = () => get(K.COMMON_PORTIONS, [50, 100, 150, 200])
export const saveCommonPortion = (grams) => {
  const value = Math.max(10, Math.min(500, Number(grams) || 100))
  const next = [value, ...getCommonPortions().filter((item) => Number(item) !== value)].slice(0, 8)
  set(K.COMMON_PORTIONS, next)
  trackEvent('common_portion_saved', { grams: value })
  return next
}

export const getChartTags = (date = todayStr()) => get(K.CHART_TAGS, {})[date] || []
export const toggleChartTag = (date = todayStr(), tag) => {
  if (!tag) return getChartTags(date)
  const all = get(K.CHART_TAGS, {})
  const current = new Set(all[date] || [])
  if (current.has(tag)) current.delete(tag)
  else current.add(tag)
  const next = { ...all, [date]: Array.from(current) }
  set(K.CHART_TAGS, next)
  trackEvent('chart_tag_toggle', { date, tag })
  notifyDataChanged('chart-tags')
  return next[date]
}
...
export const getSyncStatus = () => {
  const pending = getSyncPendingState()
  const meta = getSyncMetaState()
  return {
    ...meta,
    pendingCount: getSyncPendingCount(pending),
    pendingByTable: SYNC_TABLES.reduce((acc, table) => {
      acc[table] = pending[table].length
      return acc
    }, {}),
  }
}
...
export const retryPendingSync = async () => {
  const pending = getSyncPendingState()
  const pendingCount = getSyncPendingCount(pending)
  if (!pendingCount) return { ok: true, count: 0, pendingCount: 0 }

  const t = token()
  if (!t) {
    const message = '当前未登录，无法将新增记录同步到数据库'
    failSync({ table: 'all', operation: 'manual_push', message })
    return { ok: false, count: 0, pendingCount, message }
  }

  updateSyncMeta({ syncing: true, lastOperation: 'manual_push', lastFailureMessage: '' })
  notifySyncState({ table: 'all', operation: 'manual_push', status: 'syncing', pendingCount })
  ...
  for (const table of SYNC_TABLES) {
    ...
    const rows = rowsByTable[table].filter((row) => idSet.has(normalizeSyncId(row.id)))

    if (!rows.length) {
      resolvePendingSync(table, ids)
      continue
    }

    const ok = await syncFetchWithRetry(table, 'manual_push', `/api/sync/${table}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${t}` },
      body: JSON.stringify({ rows }),
    }, 3, {
      onSuccess: () => resolvePendingSync(table, rows.map((row) => row.id)),
    })

    if (ok) syncedCount += rows.length
    else failedCount += rows.length
  }
...
export const duplicateDietFromDate = (fromDate) => {
  ...
  const t = token()
  if (t && newRows.length) {
    const pending = queuePendingSync('diet', newRows.map((row) => row.id))
    updateSyncMeta({ syncing: true, lastOperation: 'batch_push' })
    notifySyncState({ table: 'diet', operation: 'batch_push', status: 'syncing', pendingCount: getSyncPendingCount(pending) })
    syncFetchWithRetry('diet', 'batch_push', '/api/sync/diet', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${t}` },
      body: JSON.stringify({ rows: newRows }),
    }, 3, {
      onSuccess: () => resolvePendingSync('diet', newRows.map((row) => row.id)),
    })
  } else if (newRows.length) {
    queuePendingSync('diet', newRows.map((row) => row.id))
    failSync({ table: 'diet', operation: 'batch_push', message: '当前未登录，新增记录仅保存在本地' })
  }
  return srcs.length
}
